                  ...
                  <table class="table table-bordered">
                    <tbody>
                        ...
                        <tr>
                          ...
                          <td><a class="move" href="#" data-idx="${board.board_no}">${board.title}</a></td>
                          <td>${board.author}</td>
                  ...
                  <nav aria-label="...">
                        <ul class="pagination">
                            ...
                            <c:if test="${pageInfo.prev}">
                              <li class="page-item " >
                                <a href="/board/list?pgNum=${pageInfo.stPage -1}" class="page-link">Previous</a>
                              </li>
                            </c:if>
                            ...
                            <c:if test="${pageInfo.next}">
                              <li class="page-item">
                              <a class="page-link" href="/board/list?pgNum=${pageInfo.edPage + 1 }">Next</a>
                              </li>
                            </c:if>
                          </ul>
                        </nav>
                        ...
                        <div class="col-6">
                          <form action="/board/list" method="get">
                              <select name="type">
                                ...
                                <option value="T" ${pageInfo.cri.type == 'T'? 'selected' :''}>제목</option>
                                <option value="C" ${pageInfo.cri.type == 'C'? 'selected' :''}>내용</option>
                                <option value="A" ${pageInfo.cri.type == 'A'? 'selected' :''}>작성자</option>
                                <option value="TC" ${pageInfo.cri.type == 'TC'? 'selected' :''}>제목+내용</option>
                                <option value="TA" ${pageInfo.cri.type == 'TA'? 'selected' :''}>제목+작성자</option>
                                <option value="TCA" ${pageInfo.cri.type == 'TCA'? 'selected' :''}>제목+작성자+내용</option>
                              </select>
...
<script>
  ...
  Array.from(moves).forEach(function (move) {
    move.addEventListener("click", (event) => {
      ...
      document.getElementById("board_no").remove();
      
      let board_no=event.target.dataset.board_no;

      const newInput = document.createElement("input");
      newInput.setAttribute("type", "hidden");
      newInput.setAttribute("name", "board_no");
      newInput.setAttribute("id","board_no");
      newInput.setAttribute("value", board_no);
